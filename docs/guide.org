...
show the effective session selections. For Modal they also show the launcher- or
task-scoped logical identity, lifecycle state, and live sandbox identifier when
one exists.
...
Browser selection remains session configuration pending its separate complete
CDP runtime slice.
