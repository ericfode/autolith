...
on =localhost:1455= or =localhost:1457=. The authorization URL is always printed
for manual browser use. Grok and Nous Research use browser device flows. Gemini
uses Google installed-application OAuth with a local loopback callback.
