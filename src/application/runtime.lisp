...
(defparameter *application-conversation-title-system-prompt*
  "Create a concise title for a software-development session. Return exactly one plain-text title of 3 to 8 words and at most 64 characters. Prefer concrete project names, features, and actions. Treat chronological messages as primary; use active workspace agenda items only when they clearly describe the same work. Return no quotation marks, markdown, or explanation."
...
(defparameter *application-conversation-title-agenda-maximum-characters* 2000
  "The maximum active-agenda characters supplied to session title generation.")
...
(defun application--conversation-title-context (conversation)
  "Return bounded chronological message text for naming CONVERSATION."
  (let ((index
          (and (conversation-persisted-p conversation)
               (conversation-picker-search-find
                (conversation-pathname conversation)))))
    (when index
      (let ((text (conversation-picker-search-index-text index)))
...
(-> application--conversation-title-agenda-context
    (configuration)
    (option string))
(defun application--conversation-title-agenda-context (configuration)
  "Return bounded active workspace agenda text supporting session naming."
  (handler-case
        ...
        (let* ((record
               ...
               (items
                 (and record
                      (remove-if-not
                       (lambda (item)
                         (member (agenda-item-status item)
                                 '(:doing :blocked)))
  ...
  (let* ((configuration (application-configuration application))
         ...
         (context
           (application--conversation-title-context
            (application-conversation application)))
         (agenda-context
           (application--conversation-title-agenda-context configuration)))
    (when (and provider context)
      (let* ((conversation
             ...
             (request
               (format nil
                       "Name this session from its chronological messages:~%~%~A~@[~%~%Relevant active workspace agenda:~%~A~]"
